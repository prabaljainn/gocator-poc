"""Dataset builder for multi-modal (Intensity + Height + 3D Gradient) YOLOv8 sunflower detector.

Extracts frames from .rec files and creates 3-channel composite images:
...
def make_fused_image(z_mm, i, valid):
    ...
    # Normalized relative elevation (0 to 250 mm mapped to 0..255)
    z_rel = np.where(valid, np.clip(z_mm - bg_z, 0, 250), 0.0)
    ch1 = (z_rel / 250.0 * 255.0).astype(np.uint8)

    # 3D surface gradient (highlights depth steps between flower heads and adjacent foliage)
...
def extract_and_label_recording(rec_path, out_dir, augment=True):
    ...
    for k in range(len(ends)):
        ...
        # Build 3D multi-modal fused image
...
def build_dataset(rec_paths, dataset_dir="data/dataset"):
    ...
    print(f"\nMulti-modal 3D dataset build complete! ({total_samples} samples, {total_heads} heads)")
